...
4. Review each generated email (they're editable, so fix anything the parser guessed
...
6. Click **Mark as sent** once it's out the door. The card collapses to a single line
   so you can track progress through a batch; click the line again to expand it.
...
- **Date & month**: the "estimated dispatch date – week of" cell, expanded to the
  Monday–Friday of that week (e.g. `21/09/26` becomes `September 21st – 25th`).
...
- **Suburb, state & postcode**: the full delivery address (not just the suburb, state,
  and postcode, despite the placeholder's name) if the address cell has a separate
